    ...
    if let Data::Struct(ref data) = input.data {
        ...
        impl Serialize for #struct_name {
            fn serialize(&self, serializer: &mut impl Serializer) -> SerializerNode {
                let node = SerializerNode::Component{
                    f_name: #name_str,
                    foundry_type_id: 1,
...
/*
// TODO: change implementation for gameobject vs struct
#[proc_macro_derive(SerializeGameobject)]
pub fn serialize_object_macro(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
