...
impl VulkanContext {
    ...
    fn create_ui_frame_buffers(&mut self) {
        let Some(ui_render_pass) = self.ui_render_pass else {
            panic!("No render_pass when calling create_frame_buffers");
        };
        let Some(swapchain_extent) = self.swap_chain_extent_used else {
        ...
        for i in 0..frame_buffer_count {
            ...
            unsafe {
                match logical_device.create_framebuffer(&frame_buffer_info, None) {
                    Ok(frame_buffer) => self.ui_frame_buffers.push(frame_buffer),
                    Err(e) => panic!("{}", e),
                };
            }
        }
                ...
                match logical_device.map_memory(
                    ...
                    Ok(memory_pointer) => {
                        ...
                        self.transform_buffers_mapped
                            .push(memory_pointer as *mut glm::Mat4x4);
                    }
                    Err(e) => panic!("{}", e),
                }
            }
        ...
        unsafe {
            ...
            unsafe {
                let ui_render_pass_info = vk::RenderPassBeginInfo {
                    render_pass: ui_render_pass,
                    framebuffer: ui_frame_buffers[image_index],
                    render_area: vk::Rect2D {
                        offset: vk::Offset2D { x: 0, y: 0 },
                        extent: swapchain_extent,
                    },
